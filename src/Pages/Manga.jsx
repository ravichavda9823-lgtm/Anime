...
function Manga() {
  ...
  async function FetchPageWiseManga() {
    try {
      let manga = await (
        await fetch(`https://api.jikan.moe/v4/manga?page=${currentPage}`)
      ).json();
      setManga(manga.data);
      setTotalCount(manga["pagination"]["items"]["total"]);
      setCurrentCount(manga["pagination"]["items"["count"]]);
      setLastPage(manga["pagination"]["last_visible_page"]);
    } catch (error) {
          ...
          <div className="container-fluid">
            <div className="row">
              ...
              <div className="col-xxl-9">
                <p className="sm-title mb-40">{currentCount} items {totalCount}</p>
                <div className="row">
                  {manga ? (
                    <>
                      {manga.map((value) => {
